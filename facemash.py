...
#converts points to type float64,
#defines and then subtracts the mean from all values along the first axis
#defines and then divides by the standard deviation from all values along the first axis
#
...
# folder = sys.argv[0]
folder= '/Users/mattwysocki/Desktop/trumpStore/build/images'

count = 0
for img_file in os.listdir(folder):
    # if str(img_file).find('img'):
    #     filename = os.path.join(folder,img_file)
    #     basis=os.path.splitext(filename)
    #     new_name=filename+'.jpg'
    #     resetName=os.rename(filename,new_name)

    if img_file.startswith(".DS"): ## also check if the element isn't an image
        continue
    if str(img_file).find('jpg')== -1:
        continue
